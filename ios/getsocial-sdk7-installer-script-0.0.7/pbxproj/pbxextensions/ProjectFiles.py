...
class FileOptions:
    ...
    def __init__(self, create_build_files=True, weak=False, ignore_unknown_type=False, embed_framework=True,
                 code_sign_on_copy=True, only_embed_framework=False):
        """
        ...
        self.only_embed_framework = only_embed_framework
...
class ProjectFiles:
    ...
    def add_file(self, path, parent=None, tree=TreeType.SOURCE_ROOT, target_name=None, force=True, file_options=FileOptions()):
            ...
            for section in self.objects.get_sections():
                for obj in self.objects.get_objects_in_section(section):
                    if u'path' in obj and ProjectFiles._path_leaf(path) == ProjectFiles._path_leaf(obj.path):
                        if obj[u'name'] is not None and len(obj[u'name']) != 0 and obj[u'name'].endswith('.framework'):
                            file_options.only_embed_framework = True
                            file_ref = self.get_files_by_name(obj[u'name'], parent)[0]
                            file_ref.set_last_known_file_type = u'wrapper.framework'
                            expected_build_phase = "PBXFrameworksBuildPhase"
        ...
        if not file_options.only_embed_framework:
        ...
        for target in self.objects.get_targets(target_name):
            ...
            for target_build_phase in build_phases:
                if not file_options.only_embed_framework:
                    ...
                    for build_file in target_build_phase.files:
                        build_file_ref = self.objects[build_file]
                        file_system_ref = self.objects[build_file_ref[u'fileRef']]
                        if file_system_ref.get_id() == file_ref.get_id():
